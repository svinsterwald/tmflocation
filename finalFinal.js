...
flatpickr("#location-duree", config);
flatpickr("#agence-duree", config);
...
const createAndAddInputs = (card, index, hook) => {
  ...
  <input type="text" name="produit[${card.slug}][id]" nature="id">
  `;
...
const addItem = (card, hiddenList, tagList) => {
  ...
  buttons.forEach((button) => button.classList.remove("is-disable"));
};
...
const btnClickHandler = (button) => {
  const parentWrapper = button.closest(`.${STEP_WRAPPER_CLASS}`);
...
const autocompleteMapInputField = document.querySelector(
  ".autocomplete-map .autocomplete"
);
const mapEl = document.querySelector(".autocomplete-map .map");
const myLatLngTMF = { lat: 47.360628062886995, lng: 2.3044207284463885 };
const latField = document.querySelector(".lat-field");
const lngField = document.querySelector(".lng-field");
const markers = [];

function updatelatLngFieldsForm(lat, lng) {
  latField.value = lat;
  lngField.value = lng;
}

function getLatLng(map, address, isNewMarker) {
  const location = address;
  axios
    .get("https://maps.googleapis.com/maps/api/geocode/json", {
      params: {
        address: location,
        key: "AIzaSyBT5an-go_G7OG9AHMuX_hDKhbAjpFwvtc",
      },
    })
    .then((response) => {
      const lat = response.data.results[0].geometry.location.lat;
      const lng = response.data.results[0].geometry.location.lng;
      returnedLatLng = { lat, lng };
      return returnedLatLng;
    })
    .then((isNewMarker) => {
      newMarkerAndPan(returnedLatLng, map);
      updatelatLngFieldsForm(returnedLatLng.lat, returnedLatLng.lng);
      return returnedLatLng;
    })
...
function initAutocomplete() {
  const autocompleteField = new google.maps.places.Autocomplete(
    autocompleteMapInputField,
    {
      types: ["geocode"],
      compenentRestrictions: { country: ["FR", "CH", "BE"] },
      fields: ["place_id", "geometry", "name"],
    }
  );
  autocompleteField.addListener("place_changed", () => {
    const address = autocompleteMapInputField.value;
    getLatLng(map, address, true);
  });
}

function newMarkerAndPan(myLatLng, map) {
  hideMarkers();
  ...
  marker.addListener("dragend", (e) => {
    ...
    updatelatLngFieldsForm(lat, lng);
  });
}

function initMap() {
  const mapComponent = mapEl;
  var options = {
    center: myLatLngTMF,
    zoom: 7,
  };

  map = new google.maps.Map(mapComponent, options);
}
...
class CompanyItem {
  ...
  render(hook) {
    const cardTemplate = `
    <h4>${this.company.info.uniteLegale.denominationUniteLegale}</h4>
